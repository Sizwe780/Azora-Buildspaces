...
const HAS_ENV_CITADELSM_ENDPOINT = Boolean(
  process.env.CITADELSM_ENDPOINT?.trim(),
);
const HAS_ENV_CITADELSG_ENDPOINT = Boolean(
  process.env.CITADELSG_ENDPOINT?.trim(),
);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const useCitadels =
      (process.env.CODE_CHAMBER_PROVIDER === "citadelsm" ||
        HAS_ENV_CITADELSM_ENDPOINT) &&
      Boolean(CITADELSM_ENDPOINT);

    // High complexity actions use Citadels G (Gemma) if available
    const highComplexityActions: ActionType[] = [
      "analyze",
      "refactor",
      "debug",
    ];
    const useCitadelsG =
      HAS_ENV_CITADELSG_ENDPOINT &&
      highComplexityActions.includes(action) &&
      Boolean(CITADELSG_ENDPOINT);
    ...
    if (useCitadels || useCitadelsG) {
      try {
        ...
        console.warn(
          "[Code Chamber AI] Citadels backend unavailable, falling back to OpenAI:",
          toSafeErrorMessage(citadelsError),
        );
      }
    }
