 ...
 *
 * Achievements are derived from the Collectible model:
 * - Each Collectible row has an `achievements` String[] field listing which
 *   achievement IDs it represents.
 * - Unlocking an achievement creates (or updates) a Collectible record owned
 *   by the authenticated user with the achievement ID in its `achievements` array.
 */
...
// Achievement definitions (static catalogue — no DB model needed)
...
export async function GET() {
  ...
  // Fetch all collectibles owned by this user that have achievements
  const collectibles = await prisma.collectible.findMany({
    where: { ownerId: userId, achievements: { isEmpty: false } },
    select: { achievements: true, createdAt: true },
  })

  // Flatten all unlocked achievement IDs with their earliest unlock time
  const unlockedMap = new Map<string, string>()
  for (const c of collectibles) {
    for (const achId of c.achievements) {
      if (!unlockedMap.has(achId)) {
        unlockedMap.set(achId, c.createdAt.toISOString())
      }
    }
  }

  const unlocked = Array.from(unlockedMap.entries()).map(([id, unlockedAt]) => {
    const def = ACHIEVEMENT_DEFINITIONS.find((d) => d.id === id)
    return { id, unlockedAt, room: def?.room ?? 'unknown' }
  })

  const totalPower = unlocked.reduce((sum, a) => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const potentialUnlocks = EVENT_CHECKS[event] ?? []

    // Fetch already-unlocked achievement IDs for this user
    const existing = await prisma.collectible.findMany({
      where: { ownerId: userId, achievements: { isEmpty: false } },
      select: { achievements: true },
    })
    const alreadyUnlocked = new Set(existing.flatMap((c) => c.achievements))
    ...
    for (const achievementId of potentialUnlocks) {
      ...
      // Create a collectible record representing this achievement
    ...
    const allUnlocked = new Set(allCollectibles.flatMap((c) => c.achievements))
    const totalPower = Array.from(allUnlocked).reduce((sum, id) => {
      const def = ACHIEVEMENT_DEFINITIONS.find((d) => d.id === id)
      return sum + (def?.power ?? 0)
    }, 0)
    ...
    return NextResponse.json({
      success: true,
      newUnlocks,
      totalAchievements: allUnlocked.size,
      totalPower,
    })
  } catch (error: any) {
