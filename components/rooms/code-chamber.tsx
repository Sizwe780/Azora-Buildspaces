...
import {
  ...
  loadEditorSettings,
  type EditorSettings,
} from "@/components/workspace/panels/settings-panel";
      ...
      <div key={nodeId}>
        ...
        >
          {isDir ? (
            <>
              {isOpen ? (
                <ChevronDown className="w-4 h-4 shrink-0 text-[#484f58]" />
              ) : (
                <ChevronRight className="w-4 h-4 shrink-0 text-[#484f58]" />
              )}
              {isOpen ? (
                <FolderOpen className="w-4 h-4 shrink-0 text-[#54aeff]" />
              ) : (
                <FolderClosed className="w-4 h-4 shrink-0 text-[#768390]" />
              )}
            </>
          ) : (
            <>
        ...
        {deleteConfirm === nodeId && (
          <div
            className="flex items-center gap-2 px-2 py-1.5 bg-[#1c1c1c] border-y border-[#30363d]"
            style={{ paddingLeft: `${depth * 16 + 8}px` }}
          >
            <span className="text-[12px] text-[#f85149] flex-1 truncate">
              Delete "{node.name}"?
            </span>
            <button
              onClick={() => handleDelete(nodeId)}
              className="px-2 py-0.5 text-[11px] rounded bg-[#da3633] text-white hover:bg-[#f85149]"
            >
              Yes
            </button>
            <button
    ...
    <div className="h-full flex flex-col bg-[#010409] text-[#e6edf3]">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-[#30363d] scrollbar-track-transparent">
        {rootId ? (
          ...
          <div className="p-4 text-center text-[13px] text-[#484f58]">
            No project loaded
          </div>
        )}
      </div>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setExpanded((prev: Set<string>) =>
                    new Set(prev).add(contextMenu.nodeId),
                  );
                  setContextMenu(null);
                }}
              >
...
function SearchSidebar() {
  ...
  const [results, setResults] = useState<
    { fileId: string; line: number; text: string }[]
  >([]);
  ...
  const handlePush = async () => {
    ...
    try {
      await fetch("/api/fs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ operation: "gitPush", path: ".", workspaceId }),
      });
    } catch {
      /* push error */
    }
  ...
  const handlePull = async () => {
    ...
    try {
      await fetch("/api/fs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ operation: "gitPull", path: ".", workspaceId }),
      });
    } catch {
      /* pull error */
    }
    ...
    if (!diffContent[fileName] && workspaceId) {
      try {
        const res = await fetch(
          `/api/fs?operation=gitDiff&path=${encodeURIComponent(fileName)}&workspaceId=${encodeURIComponent(workspaceId)}`,
        );
        if (res.ok) {
          const data = await res.json();
          setDiffContent((prev: Record<string, string>) => ({
            ...prev,
            [fileName]: data.diff || "No diff available",
          }));
        }
      } catch {
        setDiffContent((prev: Record<string, string>) => ({
          ...prev,
          [fileName]: "Failed to load diff",
        }));
      }
    }
  };
    ...
    <div className="h-full flex flex-col bg-[#010409]">
      ...
      <div className="px-3 flex-1 overflow-y-auto">
            ...
            {changedFiles.map((f) => (
              <div key={f.name}>
                <div className="flex items-center group">
                  ...
                  >
                    <span
                      className={`text-[11px] font-bold w-4 shrink-0 ${f.status === "A" ? "text-emerald-400" : f.status === "D" ? "text-red-400" : "text-amber-400"}`}
                    >
                      {f.status}
                    </span>
                    {getFileIcon(f.name)}
                  ...
                  <div className="flex items-center gap-0.5 pr-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => handleStageFile(f.name)}
                      className="p-1 rounded hover:bg-[#21262d] text-[#8b949e] hover:text-emerald-400"
                      title="Stage"
                    >
                      <Plus className="w-3 h-3" />
                    </button>
                    <button
                      onClick={() => handleDiscardFile(f.name)}
                      className="p-1 rounded hover:bg-[#21262d] text-[#8b949e] hover:text-red-400"
                      title="Discard"
                    >
                      <X className="w-3 h-3" />
                    </button>
        ...
        <div className="mt-4 pt-3 border-t border-[#1b1f27]">
          <div className="text-[11px] font-semibold uppercase tracking-wider text-[#8b949e] mb-2">
            Recent Commits
          </div>
          {gitError ? (
            <div className="text-center py-4">
              <p className="text-[13px] text-[#484f58] mb-2">
                No git repository
              </p>
              <button
                onClick={handleInitGit}
                className="px-3 py-1.5 rounded-md text-[12px] bg-[#238636] hover:bg-[#2ea043] text-white transition-colors"
              >
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[10px] text-[#484f58]">
                    {c.hash} ┬╖ {currentBranch}
                  </div>
                </div>
              </div>
            ))
          ) : (
            <div className="text-[13px] text-[#484f58] text-center py-4">
              No commits yet
            </div>
          )}
        </div>
...
function ExtensionsSidebar() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/code-chamber/extensions?action=search&q=${encodeURIComponent(query)}`,
        );
        if (res.ok) {
  ...
  const handleInstall = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setExtensions((prev) =>
          prev.map((e) =>
            e.id === id ? { ...e, installed: true, enabled: true } : e,
          ),
        );
      }
    } catch {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-[#484f58] truncate">
                    {ext.publisher}
                  </div>
                  <div className="text-[12px] text-[#8b949e] truncate">
                    {ext.description}
                  </div>
                </div>
              </div>
  ...
  >([
    {
      role: "assistant",
      content:
        "Hi! I'm Elara, your AI coding assistant. I can help you write, debug, and understand code. What would you like to work on?",
    },
  ]);
      ...
      if (res.ok) {
        ...
        if (data.changes && data.changes.length > 0) {
          ...
          data.changes.forEach((change: any) => {
            ...
            const fileId = Object.keys(fileMap).find(
              (id) => fileMap[id]?.name === change.path,
            );
          ...
          setMessages((prev) => [
            ...
            {
              role: "assistant",
              content: "No changes were needed based on your request.",
            },
          ]);
        }
      } else {
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content:
              "Sorry, I encountered an error while trying to refactor the code.",
          },
        ]);
      }
    } catch (error) {
  ...
  const handleClone = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const updated = [
          name,
          ...recentProjects.filter((r: string) => r !== name),
        ].slice(0, 5);
        localStorage.setItem(
          "citadel-recent-projects",
    ...
    <div className="h-full overflow-y-auto bg-[#010409] scrollbar-thin scrollbar-thumb-[#30363d] scrollbar-track-transparent">
      <div className="max-w-4xl mx-auto py-20 px-8">
        ...
        {showClone && (
          <div className="mb-8 p-5 rounded-xl bg-[#161b22] border border-[#30363d] space-y-3">
            ...
            <button
              ...
              className="w-full py-2 rounded-md text-[13px] font-medium bg-[#238636] hover:bg-[#2ea043] text-white transition-colors disabled:opacity-40"
            >
        ...
        {showNew && (
          <div className="mb-8 p-5 rounded-xl bg-[#161b22] border border-[#30363d] space-y-3">
            ...
            <button
              ...
              className="w-full py-2 rounded-md text-[13px] font-medium bg-[#1f6feb] hover:bg-[#388bfd] text-white transition-colors"
            >
              Create Project
            </button>
          </div>
        ...
        {recentProjects.length > 0 && (
          <div className="mb-12">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-[#8b949e] mb-4">
              Recent Projects
            </h2>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-3 gap-3">
            {projectTemplates.map((t) => (
              <button
                key={t.id}
                onClick={() => onProjectSelect(t.id)}
                className="flex items-center gap-3 p-4 rounded-lg bg-[#161b22] border border-[#30363d] hover:border-[#1f6feb]/50 transition-all text-left group"
              >
                <span className="text-2xl">{t.icon}</span>
                <div>
                  <div className="text-[13px] font-medium text-white group-hover:text-[#58a6ff] transition-colors">
                    {t.name}
                  </div>
...
export function CodeChamber({ id }: CodeChamberProps) {
  ...
  const [lintSummary, setLintSummary] = useState<LintResult["summary"] | null>(
    null,
  );
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [editorSettings, setEditorSettings] = useState<EditorSettings>(() =>
    loadEditorSettings(projectId),
  );
  ...
  const [codeSettings, setCodeSettings] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      return saved
        ? JSON.parse(saved)
        : { autoSave: true, showMinimap: true, theme: "vs-dark" };
    }
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("azora:settingsChanged", handleSettingsChange);
  }, []);
  ...
  const createCodeVersion = () => {
    const version = {
      ...
      files: Object.keys(fileMap).reduce(
        (acc, fileId) => {
  ...
  const restoreCodeVersion = (versionId: string) => {
    ...
    if (version) {
      ...
      Object.entries(version.files).forEach(([path, content]) => {
        const fileId = Object.keys(fileMap).find(
          (id) => fileMap[id]?.path === path,
        );
        if (fileId) {
          writeFile(fileId, content as string);
        }
      });
      setCodeDiagnostics((prev) => [
        ...prev,
        {
          id: "version-restored",
  ...
  const updateCodeSettings = (newSettings: Partial<typeof codeSettings>) => {
    ...
    window.dispatchEvent(
      new CustomEvent("azora:settingsChanged", { detail: updated }),
    );
  };
  ...
  useEffect(() => {
    ...
    saveSession(
      { sidebarView, sidebarVisible, panelView, panelVisible },
      openFiles,
      activeFileId,
    );
  }, [
  ...
  const [liveSharePeers, setLiveSharePeers] = useState<
    {
      clientId: number;
      name: string;
      color: string;
  ...
  useEffect(() => {
    ...
    getWebrtcProvider()
      .then((WebrtcProvider) => {
        ...
        setLiveShareLink(
          `${typeof window !== "undefined" ? window.location.origin : ""}/workspace/${projectId}?share=${roomName}`,
        );
      })
      .catch(() => {
  ...
  useEffect(() => {
    const handleNewFile = () => {
      ...
      if (name) {
        import("@/lib/stores/file-system").then(({ useFileSystem }) => {
          const fs = useFileSystem.getState();
          fs.createFile(null, name.trim(), "")
            .then((fileId) => {
    ...
    window.addEventListener("workspace:new-file", handleNewFile);
    return () =>
      window.removeEventListener("workspace:new-file", handleNewFile);
  }, []);
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === "s") {
        ...
        if (activeFileId) {
          ...
          if (c !== undefined) writeFile(activeFileId, c);
        }
      }
    };
        ...
        const decoration = ed.createDecorationsCollection([
          {
            ...
            options: {
              ...
              after: {
                content: " ≡ƒºá Analyzing...",
                inlineClassName: "elara-inline-hint",
              },
            },
          },
        ]);
        ...
        lintTimeoutRef.current = setTimeout(async () => {
          ...
          try {
            ...
            if (monacoRef.current && editorInstance) {
              const model = editorInstance.getModel();
              if (model) {
                const markers = (data.diagnostics || []).map(
                  (d: Diagnostic) => ({
            ...
            {diagnostics.length === 0 ? (
              <div className="text-[13px] text-[#484f58] text-center py-8">
                ...
                <p className="text-[11px] mt-1">
                  {lintSummary
                    ? `Quality score: ${lintSummary.score}/100`
                    : "Save a file to lint it"}
                </p>
              </div>
            ) : (
              <div className="py-1">
                {diagnostics.map((d, i) => (
                  ...
                  >
                    <div
                      className={cn(
                        "text-[11px] font-bold uppercase shrink-0 mt-0.5",
                        d.severity === "error"
                          ? "text-red-400"
                          : d.severity === "warning"
                            ? "text-yellow-400"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[11px] text-[#484f58] mt-0.5">
                        {activeFileName}:{d.line}
                        {d.column ? `:${d.column}` : ""} ┬╖ {d.rule}
                      </div>
                      {d.fix && (
        ...
        return (
          <div className="h-full bg-[#0d1117] p-4">
            <div className="text-[13px] text-[#484f58] text-center py-8">
              ...
              <p className="text-[11px] mt-1">Start debugging with F5</p>
            </div>
          </div>
        );
      case "diagnostics":
          ...
          <div className="h-full bg-[#0d1117] overflow-y-auto">
            <div className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button
                  size="sm"
                  onClick={createCodeVersion}
                  className="h-6 text-[11px] bg-[#238636] hover:bg-[#2ea043]"
                >
                  Create Version
                </Button>
              </div>
              {codeVersions.length === 0 ? (
                ...
                <div className="space-y-2">
                  {codeVersions.map((version: any, i: number) => (
                    <div
                      key={version.id}
                      className="flex items-center gap-3 p-3 rounded border border-[#30363d] hover:border-[#1f6feb]/50 transition-colors"
                    >
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => restoreCodeVersion(version.id)}
                        className="h-6 text-[11px]"
                      >
                        Restore
                      </Button>
          ...
          <div className="h-full bg-[#0d1117] overflow-y-auto">
            <div className="p-4 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {(["explain", "fix", "refactor", "test", "doc"] as const).map(
                  (action) => (
                    <Button
                      key={action}
                      size="sm"
              ...
              {aiActionResult && !isAiActionRunning && (
                <div className="rounded border border-[#30363d] bg-[#161b22] overflow-hidden">
                  <div className="flex items-center justify-between px-3 py-2 border-b border-[#30363d]">
                    <span className="text-[11px] text-purple-400 uppercase font-medium">
                      {aiActionType} Result
                    </span>
                    <button
                      onClick={() =>
                        navigator.clipboard.writeText(aiActionResult)
                      }
              ...
              {!aiActionResult && !isAiActionRunning && (
                <div className="text-[13px] text-[#484f58] text-center py-8">
                  ...
                  <p className="text-[11px] mt-1">
                    Or run on the entire file with no selection
                  </p>
                </div>
              )}
            </div>
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div className="h-full w-full flex flex-col bg-[#0d1117] overflow-hidden">
        ...
        <div className="h-9 flex items-center justify-between px-3 bg-[#010409] border-b border-[#1b1f27] shrink-0 select-none">
          <div className="flex items-center gap-3 text-[13px]">
            ...
            <span className="text-[#8b949e] truncate max-w-[300px]">
          ...
          <div className="flex items-center gap-1">
            <Button
              variant="ghost"
              size="sm"
              className="h-6 px-2 text-[11px] text-[#8b949e] hover:text-white hover:bg-[#30363d] gap-1.5"
              onClick={() => {
            ...
            >
              <Globe className="w-3 h-3 text-[#54aeff]" />
              Deploy
            </Button>
            <Button
              variant="ghost"
                  ...
                  if (editorInstance) {
                    editorInstance.updateOptions({
                      ...
                      bracketPairColorization: {
                        enabled: s.bracketPairColorization,
                      },
                  ...
                  >
                    <div className="h-full flex flex-col bg-[#0d1117]">
                      ...
                      <div
                        ...
                        className="flex items-center bg-[#010409] border-b border-[#1b1f27] overflow-x-auto shrink-0 scrollbar-none min-h-[35px]"
                      >
                        {openFiles.map((fId) => {
                      ...
                      {/* Breadcrumb bar */}
                      {activeFileName && (
                        <BreadcrumbBar fileName={activeFileName} />
                      )}
        ...
        <IDEStatusBar
          ...
          cursorCol={cursorCol}
          errorCount={
            diagnostics.filter((d: Diagnostic) => d.severity === "error").length
          }
          warningCount={
            diagnostics.filter((d: Diagnostic) => d.severity === "warning")
              .length
          }
        />
      </div>
      ...
      {quickOpenOpen && (
        ...
        >
          <div
            className="w-[520px] bg-[#1c2128] border border-[#30363d] rounded-lg shadow-2xl overflow-hidden"
            onClick={(e: React.MouseEvent) => e.stopPropagation()}
          >
            <input
              autoFocus
            ...
            <div className="max-h-[300px] overflow-y-auto">
              {Object.values(fileMap)
                .filter(
                  (n: any) =>
                    n.type === "file" &&
                    n.name.toLowerCase().includes(quickOpenQuery.toLowerCase()),
                )
                .slice(0, 15)
                .map((n: any) => (
                  <button
                  ...
                  >
                    ...
                    <span className="text-[11px] text-[#484f58] truncate max-w-[200px]">
                      {n.path}
                    </span>
                  </button>
                ))}
              {Object.values(fileMap).filter(
                (n: any) =>
                  n.type === "file" &&
                  n.name.toLowerCase().includes(quickOpenQuery.toLowerCase()),
              ).length === 0 && (
                  <div className="px-4 py-6 text-center text-[13px] text-[#484f58]">
                    No files found
                  </div>
                )}
            </div>
