...
function CodeBlockRenderer({ language, code, filename }: CodeBlock) {
  ...
  return (
    <div className="my-3 rounded-lg border border-border/50 overflow-hidden bg-muted/80">
...
function ThinkingIndicator({ agent }: { agent?: string }) {
  return (
    <motion.div
      ...
      className="flex flex-col gap-3 px-6 py-6 border-b border-border/30 bg-purple-500/[0.02]"
    >
      ...
      <motion.div
        ...
        className="ml-13 mt-2 rounded-2xl border border-border/60 bg-background/80 backdrop-blur-md overflow-hidden max-w-lg shadow-2xl"
      >
        <div className="px-4 py-2 border-b border-border/80 bg-muted/40 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <Brain className="w-4 h-4 text-purple-400" />
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Agent Reasoning Trace</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-[9px] text-muted-foreground font-mono tracking-tighter">LIVE STREAM</span>
          </div>
        </div>
...
function DevOpsNavigator() {
  ...
  if (loading) return <div className="p-4 text-xs text-muted-foreground animate-pulse font-mono">Scanning Infrastructure...</div>;
  ...
  return (
    <div className="p-4 space-y-6">
      <div className="space-y-3">
        ...
        <div className="grid gap-2">
          {health?.services?.map((svc: any) => (
            <div key={svc.name} className="p-2.5 rounded-lg border border-border bg-muted/40 flex items-center justify-between group hover:border-border transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[9px] text-muted-foreground font-mono uppercase tracking-tighter">{svc.type || 'container'}</span>
                </div>
              </div>
              <Badge variant="outline" className="text-[9px] border-border text-muted-foreground group-hover:text-zinc-300">
      ...
      <div className="space-y-3 pt-2">
        ...
        <div className="p-3 rounded-lg border border-dashed border-border bg-background flex flex-col items-center justify-center text-center">
              ...
              {showActions && (
                ...
                >
                  <Button
                    ...
                    className="h-7 px-2 text-xs text-muted-foreground hover:text-zinc-200"
                  >
                    {copied ? <Check className="w-3 h-3 mr-1" /> : <Copy className="w-3 h-3 mr-1" />}
                    {copied ? "Copied" : "Copy"}
                  </Button>
                  <Button
                    ...
                    className={`h-7 px-2 text-xs ${message.rating === "up" ? "text-emerald-400" : "text-muted-foreground hover:text-zinc-200"}`}
                  >
                  ...
                  <Button
                    ...
                    className={`h-7 px-2 text-xs ${message.rating === "down" ? "text-red-400" : "text-muted-foreground hover:text-zinc-200"}`}
                  >
  ...
  return (
    <div className="flex h-full bg-background">
      ...
      <AnimatePresence>
        {showHistory && (
          <motion.div
            ...
            className="border-r border-border bg-muted/50 flex flex-col overflow-hidden"
          >
            <div className="p-3 border-b border-border space-y-2">
              <Button onClick={handleNewChat} className="w-full gap-2 bg-muted hover:bg-zinc-700 text-zinc-200" size="sm">
              ...
              <div className="flex gap-1 px-1">
                <button
                  ...
                  className={`flex-1 text-[10px] font-bold uppercase tracking-wider py-2 rounded-md transition-all ${sidebarTab === "chats" ? "bg-muted text-purple-400 border border-border shadow-inner" : "text-muted-foreground hover:text-zinc-300"}`}
                >
                  Chats
                </button>
                <button
                  onClick={() => setSidebarTab("history")}
                  className={`flex-1 text-[10px] font-bold uppercase tracking-wider py-2 rounded-md transition-all ${sidebarTab === "history" ? "bg-muted text-purple-400 border border-border shadow-inner" : "text-muted-foreground hover:text-zinc-300"}`}
                >
                  History
                </button>
                <button
                  onClick={() => setSidebarTab("infra")}
                  className={`flex-1 text-[10px] font-bold uppercase tracking-wider py-2 rounded-md transition-all ${sidebarTab === "infra" ? "bg-muted text-emerald-400 border border-border shadow-inner" : "text-muted-foreground hover:text-zinc-300"}`}
                >
                  Infra
                </button>
              </div>
                ...
                <div className="p-2 space-y-1">
                  {sessions.map((session) => (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg text-sm transition-colors hover:bg-muted/50 ${sessionId === session.id ? "bg-muted/70 text-white" : "text-muted-foreground"
                        }`}
                    >
                  ...
                  <div className="pt-4 px-2">
                    ...
                    <div className="grid gap-2">
                      {recentSessions.map((execution) => (
                        <div key={execution.id} className="rounded-xl border border-border/60 bg-muted/60 p-3 hover:border-border transition-colors cursor-default group">
                          <div className="flex items-center justify-between">
                            <div className="min-w-0 pr-2">
                              ...
                              <div className="flex items-center gap-2 mt-1.5">
                                ...
                                <span className="text-[9px] text-muted-foreground font-mono uppercase tracking-tighter font-bold">
                            ...
                            <Button
                              size="sm"
                              className="h-7 w-7 p-0 flex-shrink-0 bg-transparent hover:bg-emerald-500/20 text-emerald-400 border border-border hover:border-emerald-500/40"
                ...
                <div className="p-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 text-xs bg-muted border border-border rounded-lg text-zinc-300 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-500"
                    />
                  </div>
                </div>
                <ScrollArea className="flex-1">
                    ...
                    {commandHistory
                      ...
                      .map((item) => (
                        <button
                          ...
                          className="w-full text-left p-2.5 rounded-lg text-xs transition-colors hover:bg-muted/50"
                        >
                          <div className="flex items-center gap-2">
            ...
            <div className="border-t border-border p-3">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Templates</span>
              </div>
              <div className="space-y-0.5">
                {SCRIPT_TEMPLATES.map((tpl) => (
                  <button
                    ...
                    className="w-full text-left px-2.5 py-1.5 rounded-md text-xs text-muted-foreground hover:text-zinc-200 hover:bg-muted/50 transition-colors font-mono"
                  >
                    {tpl.label}
                  </button>
                ))}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Bar */}
        <div className="h-12 border-b border-border/60 flex items-center justify-between px-4 bg-muted/30">
          <div className="flex items-center gap-3">
            <Button
              variant="ghost"
              size="sm"
              ...
              <AnimatePresence>
                {showModelPicker && (
                  <motion.div
                    ...
                    className="absolute top-full left-0 mt-1 w-72 bg-muted border border-border rounded-xl shadow-2xl z-50 p-2"
                  >
                    {MODELS.map((model) => (
                      <button
                        key={model.id}
                        onClick={() => { setSelectedModel(model); setShowModelPicker(false) }}
                        className={`w-full flex items-center gap-3 p-3 rounded-lg text-left transition-colors ${selectedModel.id === model.id ? "bg-muted text-white" : "hover:bg-muted/50 text-muted-foreground"
                          }`}
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className="text-[9px] h-4 px-1 border-border">{model.badge}</Badge>
                          </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-8">
                  {QUICK_ACTIONS.map((action) => (
                    <button
                      ...
                      className="flex flex-col items-center gap-2 p-4 rounded-xl border border-border hover:border-zinc-600 hover:bg-muted/30 transition-all text-muted-foreground hover:text-zinc-200"
                    >
                ...
                <div className="space-y-2">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className="w-full text-left p-3 rounded-lg border border-border/50 hover:border-border hover:bg-muted/20 text-sm text-muted-foreground hover:text-zinc-200 transition-all"
                    >
                      {suggestion}
                    </button>
                  ))}
        ...
        <div className="border-t border-border/40 bg-background/80 backdrop-blur-sm px-4 py-3">
          <div className="max-w-3xl mx-auto">
            <div className="relative bg-muted border border-border/50 rounded-2xl focus-within:border-zinc-600 focus-within:ring-1 focus-within:ring-zinc-600/50 transition-all">
              ...
              <AnimatePresence>
                {input.startsWith("/") && !input.includes(" ") && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 4 }}
                    className="absolute bottom-full left-0 right-0 mb-2 bg-muted border border-border rounded-xl shadow-2xl z-50 p-2 max-h-64 overflow-y-auto"
                  >
                    ...
                    {(() => {
                      ...
                      return pinned.length > 0 ? (
                        <div>
                          ...
                          {pinned.map((cmd) => (
                            <div key={`pinned-${cmd.name}`} className="flex items-center group/cmd rounded-lg hover:bg-muted/50">
                              ...
                              <button
                                ...
                                className="flex-1 flex items-center gap-3 p-2.5 text-left text-muted-foreground"
                              >
                    ...
                    {SLASH_COMMANDS.filter((cmd) =>
                      ...
                      <div key={cmd.name} className="flex items-center group/cmd rounded-lg hover:bg-muted/50">
                        <button
                          onClick={() => setInput(cmd.name + " ")}
                          className="flex-1 flex items-center gap-3 p-2.5 text-left text-muted-foreground"
                        >
                        ...
                        <button
                          ...
                          className={`p-2 transition-opacity ${pinnedCommands.has(cmd.name) ? "text-purple-400 opacity-100" : "text-muted-foreground hover:text-zinc-300 opacity-0 group-hover/cmd:opacity-100"}`}
              ...
              <div className="flex items-center gap-1 px-3 pt-3">
                <Button variant="ghost" size="sm" className="h-7 px-2 text-muted-foreground hover:text-zinc-300">
