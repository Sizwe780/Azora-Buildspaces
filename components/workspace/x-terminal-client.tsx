...
function decodeImportedProfilePayload(
  rawPayload: string,
  ...
  try {
    ...
    if (
      ...
      "profile" in (parsed as Record<string, unknown>)
    ) {
...
export default function XTerminalClient({
  onData,
  socket,
  ...
  const defaultShell: TerminalShell =
    typeof navigator !== "undefined" && /Windows/i.test(navigator.userAgent)
      ? "powershell"
      : "bash";
  const activeShellRef = useRef<TerminalShell>(
    shell === "bash" || shell === "powershell" ? shell : defaultShell,
  );
  const profileScopeRef = useRef<ProfileScope>("session");
  const profileRef = useRef<TerminalProfileStore>(emptyProfileStore());
  const addLog = useWorkbenchRuntimeStore(
    (state: ReturnType<typeof useWorkbenchRuntimeStore.getState>) =>
      state.addLog,
  );
  ...
  const getProfileStorageKey = useCallback(
    (scope: ProfileScope = profileScopeRef.current) => {
      if (scope === "workspace") {
  ...
  const loadProfileForScope = useCallback(
    (scope: ProfileScope) => {
      if (typeof window === "undefined") return;
      try {
  ...
  const persistHistory = useCallback(() => {
    ...
    localStorage.setItem(
      historyKey,
      JSON.stringify(commandHistoryRef.current.slice(-200)),
    );
  }, [historyKey]);
  ...
  const persistProfile = useCallback(() => {
    if (typeof window === "undefined") return;
    localStorage.setItem(
      getProfileStorageKey(profileScopeRef.current),
  ...
  const renderPromptLine = useCallback(
    (term: Terminal, value: string, cursorPosition?: number) => {
      const nextCursor =
        typeof cursorPosition === "number"
          ? Math.max(0, Math.min(value.length, cursorPosition))
          : value.length;
  ...
  const redrawInput = useCallback(
    (term: Terminal, nextValue: string) => {
  ...
  const resolveClientCwd = useCallback(
    (current: string, target: string): string | null => {
      const nextTarget = (target || "~").trim();
      if (!nextTarget || nextTarget === "~" || nextTarget === "/") return "";

      const start = nextTarget.startsWith("/")
        ? []
        : current.split("/").filter(Boolean);
      const parts = nextTarget
        .replace(/^~\/?/, "")
        .replace(/^\/+/, "")
        .split("/")
        .filter(Boolean);
  ...
  const fetchDynamicCompletions = useCallback(
    async (prefix: string): Promise<string[]> => {
      try {
        const res = await fetch(
          `/api/workbench/runtime?action=completions&prefix=${encodeURIComponent(prefix)}`,
          { cache: "no-store" },
        );
        if (!res.ok) return [];
  ...
  const handleTabCompletion = useCallback(
    async (term: Terminal) => {
      ...
      const localMatches = BUILTIN_COMPLETIONS.filter((item) =>
        item.startsWith(prefix),
      );
      const aliasMatches = Object.keys(getActiveProfile().aliases).filter(
        (item) => item.startsWith(prefix),
      );
      const matches = Array.from(
        new Set([...dynamic, ...localMatches, ...aliasMatches]),
      );
  ...
  const handleReverseSearch = useCallback(
    (term: Terminal) => {
      const query = (
        reverseSearchQueryRef.current || commandBufferRef.current
      ).trim();
      ...
      if (
        ...
        reverseSearchMatchesRef.current.length === 0
      ) {
        reverseSearchQueryRef.current = query;
        reverseSearchMatchesRef.current = history
      ...
      reverseSearchPointerRef.current =
        (reverseSearchPointerRef.current + 1) % matches.length;
  ...
  const executeCommand = useCallback(
    async (command: string) => {
      ...
      if (trimmed === "help") {
        ...
        term.write(
          "  \x1b[33mcd [path]\x1b[0m Change current working directory\r\n",
        );
        term.write(
          "  \x1b[33mshell <bash|powershell>\x1b[0m  Switch shell\r\n",
        );
        term.write(
          "  \x1b[33malias [name=value]\x1b[0m  List or define aliases (current shell)\r\n",
        );
        term.write("  \x1b[33munalias <name>\x1b[0m  Remove alias\r\n");
        term.write(
          "  \x1b[33msetenv KEY=value\x1b[0m  Set per-shell env var\r\n",
        );
        term.write("  \x1b[33munsetenv <KEY>\x1b[0m  Remove env var\r\n");
        term.write(
          "  \x1b[33menv\x1b[0m       List current-shell env vars\r\n",
        );
        term.write(
          "  \x1b[33mprofile export\x1b[0m  Export current shell profile\r\n",
        );
        term.write(
          "  \x1b[33mprofile import [--merge|--replace] <payload>\x1b[0m  Import into current shell\r\n",
        );
        term.write(
          "  \x1b[33mprofile apply [--merge|--replace] <payload>\x1b[0m  Alias of profile import\r\n",
        );
        term.write(
          "  \x1b[33mprofile diff <payload>\x1b[0m  Preview changes for current shell\r\n",
        );
        term.write(
          "  \x1b[33mprofile show [current-shell|all-shells]\x1b[0m  Show profile snapshot\r\n",
        );
        term.write(
          "  \x1b[33mprofile copy <bash|powershell>\x1b[0m  Copy profile to target shell\r\n",
        );
        term.write(
          "  \x1b[33mprofile rename-alias <old> <new>\x1b[0m  Rename alias in current shell\r\n",
        );
        term.write(
          "  \x1b[33mprofile unset-all-env [current-shell|all-shells]\x1b[0m  Clear env vars only\r\n",
        );
        term.write(
          "  \x1b[33mprofile unset-all-aliases [current-shell|all-shells]\x1b[0m  Clear aliases only\r\n",
        );
        term.write(
          "  \x1b[33mprofile clean [current-shell|all-shells]\x1b[0m  Clear aliases and env vars\r\n",
        );
        term.write(
          "  \x1b[33mprofile reset [current-shell|all-shells]\x1b[0m  Clear profile data\r\n",
        );
        term.write(
          "  \x1b[33mprofile scope [session|workspace]\x1b[0m  Get/set profile scope\r\n",
        );
        term.write("  \x1b[33mCtrl+R\x1b[0m reverse-search history\r\n");
        term.write(
          "  Any other command is executed in the workspace shell.\r\n\r\n",
        );
        writePrompt(term);
        isExecutingRef.current = false;
        return;
      ...
      if (trimmed.startsWith("profile scope ")) {
        const requestedScope = trimmed
          .slice("profile scope ".length)
          .trim()
          .toLowerCase();
      ...
      if (trimmed === "profile export") {
        ...
        term.write(
          "\x1b[2mCopy this token and run profile import/apply <payload> (or --merge) in another session.\x1b[0m\r\n",
        );
        writePrompt(term);
        isExecutingRef.current = false;
        return;
      ...
      if (
        ...
        const isApplyAlias =
          trimmed === "profile apply" || trimmed.startsWith("profile apply ");
        ...
        if (!payload) {
          const usageCommand = isApplyAlias
            ? "profile apply"
            : "profile import";
          term.write(
          ...
          mode === "merge"
            ? sanitizeProfile({
                aliases: {
                  ...
                  ...importedProfile.aliases,
                },
                envVars: {
                  ...activeProfile.envVars,
                  ...importedProfile.envVars,
                },
              })
            : importedProfile;
      ...
      if (trimmed === "profile show" || trimmed.startsWith("profile show ")) {
        ...
        if (target === "all-shells") {
          ...
          for (const shellName of ["bash", "powershell"] as const) {
            ...
            const aliasKeys = Object.keys(shellProfile.aliases).sort((a, b) =>
              a.localeCompare(b),
            );
            const envKeys = Object.keys(shellProfile.envVars).sort((a, b) =>
              a.localeCompare(b),
            );
            term.write(`\r\n${shellName}\r\n`);
        ...
        const activeShell = activeShellRef.current;
        const activeProfile = getActiveProfile();
        const aliasKeys = Object.keys(activeProfile.aliases).sort((a, b) =>
          a.localeCompare(b),
        );
        const envKeys = Object.keys(activeProfile.envVars).sort((a, b) =>
          a.localeCompare(b),
        );
      ...
      if (trimmed === "profile reset" || trimmed.startsWith("profile reset ")) {
        ...
        if (target === "all-shells") {
          const aliasCount =
            Object.keys(profileRef.current.bash.aliases).length +
            Object.keys(profileRef.current.powershell.aliases).length;
          const envCount =
            Object.keys(profileRef.current.bash.envVars).length +
            Object.keys(profileRef.current.powershell.envVars).length;
          profileRef.current = emptyProfileStore();
          persistProfile();
          term.write(
      ...
      if (trimmed === "profile copy" || trimmed.startsWith("profile copy ")) {
        const targetRaw =
          trimmed === "profile copy"
            ? ""
            : trimmed.slice("profile copy ".length).trim().toLowerCase();
      ...
      if (
        trimmed === "profile rename-alias" ||
        trimmed.startsWith("profile rename-alias ")
      ) {
        const rawArgs =
          trimmed === "profile rename-alias"
            ? ""
            : trimmed.slice("profile rename-alias ".length).trim();
        const [oldAlias, newAlias, ...extraArgs] = rawArgs
          .split(/\s+/)
          .filter(Boolean);
        ...
        if (
          !ALIAS_NAME_PATTERN.test(oldAlias) ||
          !ALIAS_NAME_PATTERN.test(newAlias)
        ) {
          term.write(
      ...
      if (
        trimmed === "profile unset-all-env" ||
        trimmed.startsWith("profile unset-all-env ")
      ) {
        const target =
          trimmed === "profile unset-all-env"
            ? "current-shell"
            : trimmed
                .slice("profile unset-all-env ".length)
                .trim()
                .toLowerCase();
      ...
      if (
        trimmed === "profile unset-all-aliases" ||
        trimmed.startsWith("profile unset-all-aliases ")
      ) {
        const target =
          trimmed === "profile unset-all-aliases"
            ? "current-shell"
            : trimmed
                .slice("profile unset-all-aliases ".length)
                .trim()
                .toLowerCase();
    ...
    useEffect(() => {
      ...
      if (trimmed.startsWith("alias ")) {
        ...
        if (
          ...
          Object.keys(activeProfile.aliases).length >= MAX_PROFILE_ALIASES
        ) {
          term.write("\r\n\x1b[31mAlias limit reached\x1b[0m\r\n");
          writePrompt(term);
          isExecutingRef.current = false;
      ...
      if (trimmed.startsWith("setenv ")) {
        ...
        const activeProfile = getActiveProfile();
        if (
          !(key in activeProfile.envVars) &&
          Object.keys(activeProfile.envVars).length >= MAX_PROFILE_ENV_VARS
        ) {
          term.write("\r\n\x1b[31mEnv var limit reached\x1b[0m\r\n");
          writePrompt(term);
          isExecutingRef.current = false;
      ...
      if (commandForExecution !== trimmed) {
        ...
        addLog({
          source: "terminal",
          level: "info",
          message: `alias expansion: ${commandForExecution}`,
        });
      }
      ...
      try {
        if (!runtimeEngine.isReady()) {
          term.write("\x1b[33mBooting WebContainer...\x1b[0m\r\n");
          await runtimeEngine.boot();
        }

        const cwdOpt = cwdRef.current
          ? cwdRef.current.startsWith("/")
            ? cwdRef.current
            : `/${cwdRef.current}`
          : undefined;

        const process = await runtimeEngine.spawn(
          "jsh",
          ["-c", commandForExecution],
          (output) => {
            if (output.type === "stdout" || output.type === "stderr") {
              const strValue = String(output.data);
              const outStr = strValue
                .replace(/\n/g, "\r\n")
                .replace(/\r\r\n/g, "\r\n");
              if (output.type === "stderr") {
                term.write(`\x1b[31m${outStr}\x1b[0m`);
              } else {
                term.write(outStr);
              }
              addLog({
                source: "terminal",
                level: output.type === "stderr" ? "error" : "log",
                message: strValue,
              });
            }
          },
        ...
        addLog({ source: "terminal", level: "error", message: e.message });
        term.write(
          `\x1b[31mFailed to execute command: ${e.message}\x1b[0m\r\n`,
        );
      }
    ...
    const initTerminal = (container: HTMLElement) => {
      ...
      try {
        ...
        try {
          ...
          /* best-effort patch — ignore if xterm internals changed */
        }
        ...
        term.write(`\x1b[36m⚡ Buildspaces Terminal (${sessionId})\x1b[0m\r\n`);
        term.write(
          `\x1b[2mType commands to run in your workspace using ${activeShellRef.current}. Type "help" for info.\x1b[0m\r\n\r\n`,
        );
        writePrompt(term);
        ...
        term.attachCustomKeyEventHandler((e: KeyboardEvent) => {
          ...
          if (
            ...
            window.dispatchEvent(
              new CustomEvent("terminal:toggleSearch", {
                detail: { sessionId },
              }),
            );
            return false;
        ...
        const handleSearch = (ev: Event) => {
          ...
          if (!sa) return;
          if (detail.action === "findNext") {
            sa.findNext(detail.query || "", {
              regex: false,
              wholeWord: false,
              caseSensitive: false,
            });
          } else if (detail.action === "findPrevious") {
            sa.findPrevious(detail.query || "", {
              regex: false,
              wholeWord: false,
              caseSensitive: false,
            });
        ...
        term.onData((data: string) => {
          ...
          if (data === "\r") {
            ...
            // Ctrl+E => end of line
            const moveRight =
              commandBufferRef.current.length - cursorPositionRef.current;
            if (moveRight > 0) {
              term.write(`\x1b[${moveRight}C`);
              cursorPositionRef.current = commandBufferRef.current.length;
            }
            ...
            const cursor = cursorPositionRef.current;
            const next =
              current.slice(0, cursor) + data + current.slice(cursor);
            renderPromptLine(term, next, cursor + data.length);
            historyIndexRef.current = commandHistoryRef.current.length;
            clearReverseSearchState();
          }
