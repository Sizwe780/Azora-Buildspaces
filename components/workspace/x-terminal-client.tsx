  ...
  const executeCommand = useCallback(
    async (command: string) => {
      ...
      if (trimmed.startsWith("shell ")) {
        const payload = trimmed.slice("shell ".length).trim();
        if (payload === "bash" || payload === "powershell") {
          activeShellRef.current = payload;
          onShellChange?.(payload);
          term.write(`\r\n\x1b[32mSwitched shell to ${payload}\x1b[0m\r\n`);
          writePrompt(term);
        } else {
          term.write(
