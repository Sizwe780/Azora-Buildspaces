...
interface FileSystemState {
  ...
  externalChanges: Record<
    string,
    { timestamp: number; action: "modified" | "deleted" | "created" }
  ...
  loadProject: async (_projectId: string) => {
    ...
    try {
      const treeRes = await fetch(
        `/api/fs/tree?workspaceId=${encodeURIComponent(_projectId)}`,
      );
      if (treeRes.ok) {
        ...
        const templateMatch = projectTemplates.find(
          (t) => t.id === _projectId || _projectId.includes(t.id),
        );
        if (templateMatch) {
          ...
          if (scaffoldRes.ok) {
            ...
            if (retryRes.ok) {
              ...
              if (retryData.entries?.length > 0) {
                const { rootId, fileMap } = buildFileMapFromTree(
                  retryData.entries,
                );
                set({ fileMap, rootId, isLoading: false });
                // Load file contents
      ...
      console.warn(
        "[file-system] No backend data available — initializing empty workspace",
      );
      const { rootId, fileMap } = buildFileMapFromSnapshot([]);
      set({ fileMap, rootId, isLoading: false });
    } catch (error) {
  ...
  checkForExternalChanges: async () => {
    ...
    try {
      const response = await fetch(
        `/api/fs/tree?workspaceId=${encodeURIComponent(state.workspaceId)}`,
      );
      ...
      for (const watchedPath of state.fileWatchers) {
        ...
        const nodeId = Object.keys(state.fileMap).find(
          (id) => state.fileMap[id]?.path === watchedPath,
        );
        ...
        if (!current && node) {
          ...
          if (
            current.size !== (node.content?.length || 0) ||
            (current.mtime &&
              (!node.lastModified || current.mtime > node.lastModified))
          ) {
            changes[watchedPath] = {
              timestamp: Date.now(),
              action: "modified",
