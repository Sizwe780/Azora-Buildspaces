...
export type SidebarView = 'explorer' | 'outline' | 'search' | 'git' | 'extensions' | 'chat' | 'ai-assistant' | 'code-analysis' | 'refactoring' | 'snippets' | 'themes' | 'cloud' | 'cicd' | 'web3' | 'packages' | 'security' | 'figma' | 'qa-testing' | 'telemetry' | 'observability' | 'deployment' | 'settings' | 'task-runner'
export type SecondarySidebarView = 'chat' | 'ai-assistant' | 'outline' | 'timeline' | 'copilot'
export type PanelView = 'terminal' | 'output' | 'problems' | 'debug' | 'ports' | 'testing' | 'performance' | 'code-review' | 'live-preview' | 'qa-testing' | 'telemetry' | 'observability'
...
interface WorkbenchState {
    ...
    navigationHistory: Array<{ fileId: string; line: number; column: number; timestamp: number }>
    ...
    restoreEditorState: (editorGroups: EditorGroup[], activeGroupId?: string, splitDirection?: SplitDirection) => void
    ...
    editorViewStates: Record<string, { cursorLine: number; cursorColumn: number; scrollTop: number; scrollLeft: number; folds?: number[] }>
    saveEditorViewState: (fileId: string, state: { cursorLine: number; cursorColumn: number; scrollTop: number; scrollLeft: number; folds?: number[] }) => void
    getEditorViewState: (fileId: string) => { cursorLine: number; cursorColumn: number; scrollTop: number; scrollLeft: number; folds?: number[] } | null
}
...
export const useWorkbench = create<WorkbenchState>()(
    persist(
        (set, get) => ({
            ...
            setSecondarySidebarView: (view) => set({ activeSecondarySidebarView: view }),
            toggleSecondarySidebar: () => set((state) => ({ isSecondarySidebarVisible: !state.isSecondarySidebarVisible })),
            showSecondarySidebar: (view) => set((state) => ({
    ...
    togglePanel: () => set((state) => ({ isPanelVisible: !state.isPanelVisible, isPanelMaximized: false })),
    setPanelPosition: (pos) => set({ panelPosition: pos }),
    togglePanelMaximized: () => set((state) => ({ isPanelMaximized: !state.isPanelMaximized })),
    ...
    gitAhead: 0,
    gitBehind: 0,
    setDiagnostics: (errors, warnings) => set({ diagnosticErrors: errors, diagnosticWarnings: warnings }),
    setGitBranchInfo: (branch, ahead, behind) => set({ currentGitBranch: branch, gitAhead: ahead, gitBehind: behind }),

    // Recent Files
    recentFiles: [],
    addRecentFile: (fileId, name) => set((state) => {
        const filtered = state.recentFiles.filter(f => f.fileId !== fileId)
        const newRecent = [{ fileId, name, timestamp: Date.now() }, ...filtered].slice(0, 20)
    ...
    // Custom Keybindings
    keybindings: {},
    setKeybinding: (action, shortcut) => set((state) => ({
        keybindings: { ...state.keybindings, [action]: shortcut },
    })),
    resetKeybinding: (action) => set((state) => {
    ...
    // File management
    closeFile: (fileId, groupId) => set((state) => {
        const targetGroupId = groupId || state.activeGroupId
        const groups = state.editorGroups.map(g => {
            if (g.id !== targetGroupId || !g.openFiles.includes(fileId)) return g
            const openFiles = g.openFiles.filter(f => f !== fileId)
            const activeFile = g.activeFile === fileId
                ? (openFiles.length > 0 ? openFiles[Math.max(0, g.openFiles.indexOf(fileId) - 1)] : null)
                : g.activeFile
            return { ...g, openFiles, activeFile }
        })
        return { editorGroups: groups }
    }),
    setActiveFile: (fileId, groupId) => set((state) => ({
        editorGroups: state.editorGroups.map(g =>
            g.id === (groupId || state.activeGroupId)
                ? {
                    ...g,
                    activeFile: fileId,
                    openFiles: g.openFiles.includes(fileId) ? g.openFiles : [...g.openFiles, fileId],
                }
                : g
        ),
    })),
    restoreEditorState: (editorGroups, nextActiveGroupId, nextSplitDirection) => set((state) => {
        const normalizedGroups = editorGroups.length > 0
            ? editorGroups.map((group) => ({
                id: group.id,
                activeFile: group.activeFile ?? null,
                openFiles: Array.isArray(group.openFiles) ? group.openFiles : [],
            }))
            : [{ id: 'group-1', activeFile: null, openFiles: [] }]

        const validActiveGroupId = normalizedGroups.some((group) => group.id === nextActiveGroupId)
            ? nextActiveGroupId
    ...
    toggleGoToLine: () => set((state) => ({ isGoToLineVisible: !state.isGoToLineVisible })),
    setQuickOpenVisible: (v) => set({ isQuickOpenVisible: v }),
    setGoToLineVisible: (v) => set({ isGoToLineVisible: v }),

    // Overlay Terminal
    isTerminalOverlay: false,
    toggleTerminalOverlay: () => set((state) => ({ isTerminalOverlay: !state.isTerminalOverlay })),
    ...
    editorViewStates: {},
    saveEditorViewState: (fileId, viewState) => set((state) => ({
